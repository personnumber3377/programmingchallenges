...
fn main_loop(game_map: Vec<Vec<i32>>, line_offsets: Vec<i32>, mut moves: Vec<i32>, move_lenghts: Vec<i32>, vertical_offsets: Vec<i32>, vertical_lengths: Vec<i32>) {
    ...
    for m in moves {
        ...
        for n in 0..distance {
            ...
            if vertical_offsets[cur_x] > cur_y { // underflow aka go up
                cur_y = vertical_offsets[cur_x] + vertical_lengths[cur_x]; // loop back around
            }

            if vertical_offsets[cur_x] + vertical_lengths[cur_x] < cur_y {
                cur_y = vertical_offsets[cur_x];  // overflow aka go down
            } 

            // check X

            if line_offsets[cur_y] > cur_x {
                cur_x = line_offsets[cur_y] + game_map[cur_y].len(); // loop to the right when going left
            }

            if line_offsets[cur_y] + game_map[cur_y].len() < cur_x {
                cur_x = line_offsets[cur_y]; // loop to the left when going right
            }
            ...
            x_access = cur_x - line_offsets[cur_y]; // delete the offset.

            y_access = cur_y - vertical_offsets[cur_x];
...
fn main() {
    ...
    main_loop(game_map, line_offsets, moves, move_lenghts, horizontal_offsets, horizontal_lengths);
}
