...
fn parse_cube(row_lenghts: Vec<i32>, row_offsets: Vec<i32>, vertical_lengths: Vec<i32>, vertical_offsets: Vec<i32>) {
    ...
    while loop_count != row_lenghts.len() / side_len { // the number of "squares" is the y length of the map divided by the side length.


    // check if there is a "side" on the current y_coord and x_coord
        sideways_move = 0;
        if !(row_lenghts[cur_side_y_coord] <= cur_side_x_coord && row_lenghts[cur_side_y_coord] + row_lenghts[cur_side_y_coord] >= cur_side_x_coord) {

            // no side "square" here.

            // first check going "left"

            cur_side_x_coord -= side_len;

            if !(row_lenghts[cur_side_y_coord] <= cur_side_x_coord && row_lenghts[cur_side_y_coord] + row_lenghts[cur_side_y_coord] >= cur_side_x_coord) {

                // check going right

                cur_side_x_coord +=  2*side_len; // 2 * side_len , because we moved left so to go right we want to return and then go right once.

                if !(row_lenghts[cur_side_y_coord] <= cur_side_x_coord && row_lenghts[cur_side_y_coord] + row_lenghts[cur_side_y_coord] >= cur_side_x_coord) {

                    // This shouldn't happen because this signifies there are no "squares" on this line.
        ...
        } // if the move was not modified it stays at zero so do not do anything.

        // Now we have the move.

        if sideways_move == 1 {

            cur_side = move_up(cur_side); // first go back and then go left and then go down.
        ...
        if sideways_move == 2 {
            // go right
            cur_side = move_up(cur_side); // first go back and then go right and then go down.
