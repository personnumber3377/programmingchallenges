...
def check_stuff(x,y,z,marked_cube,new_marked_stuff,index,cube):
	...
	print("marked_cube[index_thing[0],index_thing[1],index_thing[2]] == "+str(marked_cube[index_thing[0],index_thing[1],index_thing[2]]))

	if index_thing[index] == 0:
		index_thing[index] += 1

		if marked_cube[index_thing[0],index_thing[1],index_thing[2]] != 1 and cube[index_thing[0],index_thing[1],index_thing[2]] != 1:
			
			new_marked_stuff.append([index_thing[0],index_thing[1],index_thing[2]])
			marked_cube[index_thing[0],index_thing[1],index_thing[2]] = 1
...
def solve_puzzle():
	...
	cube_slice = cube_thing[slice_point]

	black_cube_slice = np.zeros((cube_slice.shape[0], cube_slice.shape[0], 4))





	outer_shape_slice = outer_shape[slice_point]

	red_outer_shape_slice = np.zeros((cube_slice.shape[0], cube_slice.shape[0], 4))
	...
	for i in range(cube_slice.shape[0]):
		...
		for j in range(cube_slice.shape[1]):
			...
			if not cube_slice[i,j]:
				print("shit")
				red_outer_shape_slice[i,j] = np.array([255,255,255,1])
	...
	for i in range(outer_shape_slice.shape[0]):
		...
		for j in range(outer_shape_slice.shape[1]):
			...
			if not outer_shape_slice[i,j]:
				print("poopoo")
				red_outer_shape_slice[i,j] = np.array([255,0,0,1])


	#result = red_outer_shape_slice.astype("bool") | black_cube_slice

	result = red_outer_shape_slice
