...
def update_width_ranges(shape_num, x_coord, y_coord):
	...
	#print("width_ranges before: "+str(width_ranges))
	...
	for i in range(shape_height):
		...
		for j in range(new_range[0], new_range[1]):
			...
			if y_coord+i not in width_ranges:
				#print("y_coord+i not in width_ranges")
...
def width_ranges_to_mat():

	thing_map = np.zeros((MAP_WIDTH, MAP_HEIGHT))

	for thing in width_ranges:

		#print("thing == "+str(thing))

		thing2 = width_ranges[thing]

		for thing3 in thing2:

			thing_map[thing3,thing] = 1


	#render_mat(thing_map)
...
def render_width_ranges():

	oof = width_ranges_to_mat()
