...
def coalesce_thing(range_list):
	...
	#print("len(range_list) == "+str(len(range_list)))
	...
	while i < len(range_list)-1:
		...
		if b[0] -1 <= a[1]:
			...
			if b[1] > a[1]:
				#print("poop")
				#print("range_list before pop: "+str(range_list))
				#range_list.pop(0)
				#range_list.pop(i)
				#range_list.pop(i)


				range_list = range_list[:i]+range_list[i+2:]
				...
				'''
				#range_list.pop(0)
				#print("range_list after pop: "+str(range_list))
				#final_thing.append([a[0], b[1]])
				#range_list = [[a[0], b[1]]]+range_list
				range_list.insert(i, [a[0], b[1]])
				#print("range_list final: "+str(range_list))
				continue
				...
				#range_list = [a]+range_list

				range_list.insert(i,a)
				continue
...
def merge_ranges(range_list):
	...
	for i in range(len(range_list)-1):
		...
		# clamp the ranges to the inclusive range 0 - 4000000
		# integer = 4000000
	...
	return coalesce_thing(range_list)
	range_list.sort(key=lambda x: x[0])

	# Now coalesce.
	#debug("range_list: "+str(range_list))
	...
	# print("final_thing == "+str(final_thing))

	#print("final_thing[:10000] == "+str(final_thing[:10000]))
...
def fill_in_range(ranges, sensor, beacon):
	...
	#print("[y_coord_sens-rad, y_coord_sens] == "+str([y_coord_sens-rad, y_coord_sens]))
	

	for y in range(y_coord_sens-rad, y_coord_sens+1):
		#print("[x_coord_sens-width,x_coord_sens+width+1] == "+str([x_coord_sens-width,x_coord_sens+width+1]))
		if y == 4000000:
			break

		#print("y == "+str(y))

		#print("[x_coord_sens-width-1,x_coord_sens+width+1] == "+str([x_coord_sens-width-1,x_coord_sens+width+1]))



		ranges[y].append([x_coord_sens-width,x_coord_sens+width]) # start, end
		width += 1


	#print("oofpoopoo")
	#print("[y_coord_sens+2, y_coord_sens+rad+1] == "+str([y_coord_sens+2, y_coord_sens+rad+1]))
	
	
	width -= 2
	for y in range(y_coord_sens+1, y_coord_sens+rad+1):
		#print("y == "+str(y))
		if y == 4000000:
			break
		ranges[y].append([x_coord_sens-width,x_coord_sens+width]) # start, end
	...
	#print("ranges after filling: "+str(ranges))
	return ranges
...
def check_spaces(sensors_and_beacons):
	...
	#debug("ranges after merge: "+str(ranges))
	#print("ranges == "+str(ranges))
	time_end = time.time()
	
	print("Merging took "+str(time_end - time_start)+" seconds.")
	...
	for i in range(len(ranges)):
		...
		if ranges[i] != [[0, 4000000]]:
			print("Found!")
			print("ranges[i] == "+str(ranges[i]))
			print("i == "+str(i))
	...
	x_coord = ranges[y_coord][0][1]+1


	print("y_coord_count == "+str(y_coord_count))


	#assert y_coord_count == 1
	...
	freq = x_coord * 4000000 + y_coord

	print("Tuning frequency: "+str(freq))
	return freq
