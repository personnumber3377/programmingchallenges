...
def construct_matrix(coordinates):
	...
	#matrix = np.zeros((1000, y_coord_max+2)) # just hardcode a thousand, because sand can now fall diagonally in the part two version
	matrix = np.zeros((1000, 1001))
	#matrix = matrix.T
	# Place lines.
	...
	# Make it such that sand does not fall to the abyss

	#matrix[-1,0:x_coord_max+2] = 1
	matrix[:,y_coord_max+2] = 1
	#matrix[0:x_coord_max+2] = 1
	render_mat(matrix)
	#matrix[y_coord_max+1,0:x_coord_max+2] = 1

	#matrix[y_coord_max+2,0:x_coord_max+2] = 1
...
def sim_sand(mat):
	...
	while True:
		...
		while True:
			...
			#assert sand_coord[0] != mat.shape[0]-1 and sand_coord[1] != mat.shape[0]-1
			assert sand_coord[0] != mat.shape[0]-1 and sand_coord[1] != mat.shape[1]-1
			...
			'''
			print("mat[sand_coord[0], sand_coord[1]] == "+str(mat[sand_coord[0], sand_coord[1]]))
			print("mat[sand_coord[0+1], sand_coord[1]] == "+str(mat[sand_coord[0+1], sand_coord[1]]))
			print("mat[sand_coord[0-1], sand_coord[1]] == "+str(mat[sand_coord[0-1], sand_coord[1]]))
			print("mat[sand_coord[0-1], sand_coord[1+1]] == "+str(mat[sand_coord[0-1], sand_coord[1+1]]))
			print("mat[sand_coord[0], sand_coord[1+1]] == "+str(mat[sand_coord[0], sand_coord[1+1]]))
			print("mat[sand_coord[0+1], sand_coord[1+1]] == "+str(mat[sand_coord[0+1], sand_coord[1+1]]))
			'''
			...
			if mat[sand_coord[0], sand_coord[1]+1]: # check if occupied
				...
				if mat[sand_coord[0]-1, sand_coord[1]+1]:
					...
					if mat[sand_coord[0]+1, sand_coord[1]+1]:
						...
						print("shit")
						if sand_coord[0] == 500 and sand_coord[1] == 0: # the sand spawner is at (500,0)
							abyss = True
							break
